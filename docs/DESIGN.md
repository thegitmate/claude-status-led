...
It is now the **primary** source: any session Claude Code reports is decided by its status alone, and the hook record for that session is ignored entirely. Hooks and transcript watching remain as a fallback for sessions it does not publish, and will be removed once this has run for a while without a wrong light. That fixes the last blind spot: **a prompt submitted and then cancelled before Claude begins replying fires no hook at all and writes no transcript entry.** Verified by registering every plausible event, including `MessageDisplay`, `UserPromptExpansion`, `TaskCompleted`, `PostToolBatch` and `PreCompact`, then cancelling a prompt: nothing whatsoever was emitted between the `UserPromptSubmit` and the next one.
